      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-slate-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">Organize your saved reading list, research papers, and tools</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {filteredBookmarks.map((b) => (
            <div
              key={b.id}
              className="group p-3.5 rounded-xl bg-white hover:bg-slate-50/80 border border-slate-200 hover:border-slate-300 transition-all flex items-start justify-between gap-3 shadow-xs"
            >
              <div
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-xs font-bold text-slate-900 group-hover:text-amber-600 truncate transition-colors">
                    {b.title}
                  </div>
                  <div className="text-[11px] text-slate-500 font-mono truncate">{b.url}</div>
                  {b.folder && (
                    <span className="inline-block mt-1 text-[10px] px-1.5 py-0.5 rounded bg-slate-100 border border-slate-200 text-slate-600 font-medium">
                      📁 {b.folder}
                    </span>
                  )}
                </div>
              ...
              <button
                ...
                className="p-1 rounded-lg hover:bg-rose-50 text-slate-400 hover:text-rose-600 transition-colors opacity-0 group-hover:opacity-100 cursor-pointer"
          ...
          <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4 z-50">
            <div className="bg-white border border-slate-200 rounded-2xl p-6 w-full max-w-md space-y-4 shadow-2xl">
              ...
              <form onSubmit={handleCreate} className="space-y-3 text-xs">
                ...
                <div>
                  <label className="block text-slate-700 font-medium mb-1">URL</label>
                  <input
                    type="text"
                    required
                    value={newUrl}
                ...
                <div>
                  <label className="block text-slate-700 font-medium mb-1">Folder / Category</label>
                  <input
                    type="text"
                    value={newFolder}
                ...
                <div className="flex items-center justify-end gap-2 pt-2">
                  <button
                    ...
                    className="px-3 py-2 rounded-xl bg-slate-100 text-slate-700 hover:bg-slate-200 font-semibold cursor-pointer"
                  >
                    Cancel
                  </button>
                  <button
